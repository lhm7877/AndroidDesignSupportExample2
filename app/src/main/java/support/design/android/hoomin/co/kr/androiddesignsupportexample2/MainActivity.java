...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupGirlsViewPager(ViewPager viewPager) {
        ...
        girlsAdapter.appendFragment(
                GirlGroupFragment.newInstance(1), "Girls 1");
        girlsAdapter.appendFragment(
                GirlGroupFragment.newInstance(1), "Girls 2");
        girlsAdapter.appendFragment(
                GirlGroupFragment.newInstance(1), "Girls 3");
        viewPager.setAdapter(girlsAdapter);
    }

    private static class GirlGroupPagerAdapter extends FragmentPagerAdapter{
        private final ArrayList<GirlGroupFragment> girlsFragment =
                new ArrayList<GirlGroupFragment>();
        ...
        public void appendFragment(GirlGroupFragment fragment, String title){
            girlsFragment.add(fragment);
            tabTitles.add(title);
        }

        @Override
        public Fragment getItem(int position) {
            return girlsFragment.get(position);
        }

        @Override
        public int getCount() {
            return girlsFragment.size();
        }
